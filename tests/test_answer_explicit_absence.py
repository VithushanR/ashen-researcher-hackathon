"""Prompt and composition regression tests; semantic rejection is not implemented.

All examples are synthetic. Injected responses demonstrate the intended grounded
output, not a claim that a live LLM or a future semantic validator has been tested.
"""
...
def test_direct_event_in_absence_evidence_remains_citable():
    ...
    result = compose_answer(state(item, question="What did Test Figure do?"), synthesize=synthesize)
...
def test_absence_and_direct_event_can_both_be_supported_claims():
    ...
    result = compose_answer(state(*items), synthesize=synthesize)
...
def test_absence_qualifiers_are_preserved(absence):
    ...
    result = compose_answer(state(evidence("absence", text=absence)), synthesize=synthesize)
